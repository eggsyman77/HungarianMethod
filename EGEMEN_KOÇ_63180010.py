...
#comment: According to false case, row and column(special) is marked
...
#comment: for zeroboolean matrix (True==0, ow==False)
...
def matrixadjust(matrix, rowscover,columnscover ):
    currentmatrix = matrix
    zeroelement_none = []
#comment: Determine the minimumnumber
...
def hungarian_method(matrix):
    ...
    currentmatrix = matrix
#comment:the first step would be subtracting operation (for every row and every column)
